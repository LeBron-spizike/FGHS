...
class GNN_Encoder(torch.nn.Module):
    ...
    def forward(self, *argv):
        if len(argv) == 4:
            ...
            raise ValueError("unmatched number of arguments.")
        #enhance
        x = self.gnn.x_embedding1(x[:, 0].long()) + self.gnn.x_embedding2(x[:, 1].long())
        atom_rep = self.gnn(x, edge_index, edge_attr)
        from torch_scatter import scatter_mean
        substruct_index = batch
        substruct_rep = scatter_mean(atom_rep, substruct_index, dim=0)
        combined_rep = torch.cat([atom_rep, substruct_rep[batch]], dim=-1)
        fusion_layer = torch.nn.Linear(combined_rep.size(-1), self.emb_dim).to(combined_rep.device)
        fused_rep = fusion_layer(combined_rep)
        graph_representation = self.pool(fused_rep, batch)
