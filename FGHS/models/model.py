...
class FGHS(nn.Module):
    def __init__(self, args, task_num, train_task_num, device):
        ...
        in_dim = args.emb_dim
        self.relation_net = TaskRelationNet(args, in_dim=in_dim,
        ...
        self.shared_fusion_fc = nn.Linear(args.emb_dim * 2, args.emb_dim).to(device)


   
    def encode_mol(self, data):
        z_struct = self.mol_encoder(data.x, data.edge_index, data.edge_attr, data.batch)
        if hasattr(data, "x_element"):
            z_element = self.mol_encoder(data.x_element, data.edge_index_element, data.edge_attr_element, data.batch_element)
            z_fused = torch.cat([z_struct, z_element], dim=-1)
            z_mol = self.shared_fusion_fc(z_fused) 
        else:
            z_mol = z_struct
        return z_mol
    ...
    def data_prepare(self, s_data, q_data, sca_adj, gro_adj, tasks):
        for dname, d in zip(["support", "query"], [s_data, q_data]):
            if d.x is None or d.edge_index is None or d.edge_attr is None:
                raise ValueError(f"[{dname}] has None in required fields")
            if d.edge_index.size(1) != d.edge_attr.size(0):
            ...
            if d.edge_index.max().item() >= d.x.size(0):
                raise IndexError(f"[{dname}] edge_index out of bounds: max {d.edge_index.max().item()}, "
                                 f"but x size is {d.x.size(0)}")
        ...
        for i, (ls, w_ls, type_ls, adj1, adj2) in enumerate(zip(edge_ls_temp, edge_w_ls_temp, edge_type_ls_temp,
            ...
            if edge_ls is None:
                ...
                edge_type_ls = torch.cat((edge_type_ls, type_ls, torch.ones(values1.shape).to(self.device),
                                          torch.tensor([2] * values2.shape[0]).to(self.device)), dim=-1)
        return X, edge_ls, edge_w_ls, edge_type_ls, tasks, origin_fuse
    ...
    def forward(self, s_data, q_data, sca_adj, gro_adj, tasks):
        X, edge_ls, edge_w_ls, edge_type_ls, tasks, origin_fuse = self.data_prepare(s_data, q_data, sca_adj, gro_adj, tasks)
        ...
        return support_logit, query_logit, tgt_s_y, tgt_q_y, graph_emb, origin_fuse
