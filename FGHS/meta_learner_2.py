...
class MetaLearner:
    ...
    def test_step(self, test_auxi_task_num=None):
        ...
        # ===== t-SNE containers =====
        tsne_task_features_before = defaultdict(list)  # encode_mol_1
        tsne_task_features_after = defaultdict(list)  # encode_mol
        ...
        for task_i in tqdm(self.test_task_range, desc='eval'):
            ...
            s_data, q_data, s_data_id, q_data_id = dataset_sampler(
                self.dataset,
                self.n_support,
                self.n_query,
                tgt_id=task_i,
            ...
            test_auxi_task_num = (
                self.test_auxi_task_num
                if test_auxi_task_num is None
                else test_auxi_task_num
            )
            ...
            mask_task = (
                    (~torch.isnan(s_data.y[:, self.train_task_range])).sum(0).bool()
                    | (~torch.isnan(q_data_.y[:, self.train_task_range])).sum(0).bool()
            )

            with torch.no_grad():
                train_graph_emb = self.task_emb_get(model, task_i, mask_task)
                auxi_tasks, _ = self.sample_auxiliary(
                    train_graph_emb,
                    test_auxi_task_num,
                    mask_task
                )
            ...
            for _ in range(self.args.inner_update_step):
                ...
                sca_adj, gro_adj = self.compute_similarity_chunk(
                    s_data_id,
                    adapt_q_data.id,
                    self.args.rel_top_k
                )
                ...
                sampled_task = torch.tensor(
                    [task_i] + auxi_tasks
                ).to(self.device)

                s_logit, _, s_label, _, *_ = model(
                    s_data,
                    adapt_q_data,
                    sca_adj,
                    gro_adj,
            ...
            # ===== 5. evaluation + backbone t-SNE (核心修改) =====
            model.eval()
            sampled_task = torch.tensor(
                [task_i] + auxi_tasks
            ).to(self.device)

            query_batch = Batch.from_data_list(q_data).to(self.device)
            sca_adj, gro_adj = self.compute_similarity_chunk(
                s_data_id,
                query_batch.id,
                self.args.rel_top_k
            ...
            with torch.no_grad():
                # ---- 用于评估（保持原逻辑）----
                s_logit, q_logit, s_label, q_label, *_ = model(
                    s_data,
                    query_batch,
                    sca_adj,
                    gro_adj,
                ...
                # ---- backbone 表示（用于 t-SNE）----
                z_before = model.encode_mol_1(query_batch)  # 创新前
                z_after = model.encode_mol(query_batch)  # 创新后

            tsne_task_features_before[task_i] = [z_before.detach().cpu()]
            tsne_task_features_after[task_i] = [z_after.detach().cpu()]
            tsne_labels[task_i] = [q_label.detach().cpu()]

            # ===== 6. AUC 计算（保持原逻辑）=====
            q_pred = torch.sigmoid(q_logit).cpu().view(-1)
            q_true = q_label.cpu().view(-1)
        ...
        return (
            np.mean(auc_scores),
            tsne_task_features_before,
            tsne_task_features_after,
            tsne_labels
        )
